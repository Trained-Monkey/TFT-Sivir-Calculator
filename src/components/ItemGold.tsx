...
import { ITEM_TYPE, ITEM_MODIFIER } from "../constants/Items";
...
export const ItemGold = () => {
    ...
    const [baseAD, ,] = context.baseAD;
    const [modifierAD, ,] = context.modifierAD;
    ...
    return (<div className="ItemGold">
        ...
        <div className="ItemGoldContainer">
            {data.map(row => (<div key={row.name} className="ItemGoldRow">
                <div style={{ width: 100 }}> {row.name} </div>
                {row.items.map(x => <div key={row.name+ "-" + x} className="ItemGoldItem">
                    <img style={checkItemIncrements(x) ? { borderRadius: 5 } : { borderRadius: 5, filter: 'grayscale(100%)' }}
                        src={"./images/items/" + x + ".png"}
                        alt=""></img>
                </div>)}
            </div>))}
        </div>
